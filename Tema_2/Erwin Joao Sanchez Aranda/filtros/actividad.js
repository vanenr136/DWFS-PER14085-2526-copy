...
function redConverter() {
    ...
    //Aqui tu codigo
    const rowSize = pixels.length;
    const columnSize = pixels[0].length;
...
function greyConverter() {
    ...
    for(let row = 0; row < rowSize; row++) {
        for (let column = 0; column < columnSize; column++) {
            let average = Math.round((pixels[row][column][0] + pixels[row][column][1] + pixels[row][column][2])/3);
...
function blackAndWhiteConverter() {
    ...
    for(let row = 0; row < rowSize; row++) {
        for (let column = 0; column < columnSize; column++) {
            ...
            let average = Math.round((pixels[row][column][0] + pixels[row][column][1] + pixels[row][column][2])/3);
            let bnValue = average<128 ? 0 : 255;
            pixels[row][column][0] = bnValue;
            pixels[row][column][1] = bnValue;
            pixels[row][column][2] = bnValue;
        }
    }
...
function scaleDown() {
    ...
    for(let row = 0; row < rowSize; row+=2) {
        let resizedRow = [];
        for (let column = 0; column < columnSize; column+=2) {
            resizedRow.push(pixels[row][column]);
        }
...
function merge(alphaFirst, alphaSecond) {
    ...
    //Aqui tu codigo
    const rowSize = catPixels.length;
    const columnSize = catPixels[0].length;

    for(let row = 0; row < rowSize; row++) {
        let newRow = [];
        for (let column = 0; column < columnSize; column++) {
            let mergedPixel = [catPixels[row][column][0]*alphaFirst + dogPixels[row][column][0]*alphaSecond,
                catPixels[row][column][1]*alphaFirst + dogPixels[row][column][1]*alphaSecond,
                catPixels[row][column][2]*alphaFirst + dogPixels[row][column][2]*alphaSecond]
